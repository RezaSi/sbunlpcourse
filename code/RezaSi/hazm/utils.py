...
default_mokassar = path.join(data_path, 'mokassar.dat')
...
def words_list_word(words_file=default_words):
	with codecs.open(words_file, encoding='utf-8') as words_file:
		items = [line.strip().split('\t') for line in words_file]
		return [item[0] for item in items if len(item) == 3]


def mokassar_list_word(words_file=default_mokassar):
	with codecs.open(words_file, encoding='utf-8') as words_file:
		items = [line.strip().split('\t') for line in words_file]
		mp = dict()
		for item in items:
...
def stopwords_list_words(words_file=default_stopwords):
	with codecs.open(words_file, encoding='utf-8') as words_file:
